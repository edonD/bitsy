...
# ── Gap analysis ───────────────────────────────────────────────────────────
#
# The "what should I change" screen. Pulls mention rates + content features
# for the target and its peers from the in-memory feature store (populated
# by the last /collect) and, optionally, fresh crawls. Ranks feature gaps
# by (magnitude × research-backed impact coefficient from the GEO paper)
# so the user gets a punch list ordered by expected lift, not by raw delta.

# Effect sizes from Aggarwal et al., KDD 2024 (GEO paper). Coefficients are
# fraction-of-baseline-visibility improvements, used only to rank gaps by
# likely payoff. Not used as forecasts.
RESEARCH_IMPACT = {
...
def _brand_mention_rate(brand: str) -> Optional[float]:
    """Pull the latest mention rate for a brand from the in-memory feature store."""
...
def _brand_content_features(brand: str, crawl_mode: str, max_pages: int) -> dict:
    """
    Pull content features for a brand. Prefers the feature store if the
    numbers look real; falls back to crawling the brand's registered
    homepage when everything is zero. Returns a dict with float values.
    """
    from pipeline.brand_domains import get_domain

    # First try the in-memory feature store
    store_row = None
    for row in _store.get("features") or []:
        if row.get("brand", "").lower() == brand.lower():
            store_row = row
    ...
    # Fall back to store values for keys the crawler couldn't measure.
    for k in content_keys:
...
def _compute_gap(feature: str, user_val: float, peer_vals: list[float]) -> Optional[dict]:
    ...
    Direction-aware: for "decrease" features (freshness_days, readability_grade)
    the leader has the smaller value, not the larger.
    """
    if not peer_vals:
        return None
    ...
    if gap <= 0:
        return None  # user is already at or ahead of the leader

    peer_avg = sum(peer_vals) / len(peer_vals)
    # Normalize gap to [0,1] range vs leader so features in different scales
    # can be compared. Tiny baseline → small normalized gap.
...
def _evidence_text(gap: dict, leader_brand: str, target_brand: str) -> str:
    """One-sentence human-readable evidence for the gap."""
...
def gap_analysis_endpoint(req: GapAnalysisRequest):
    """
    Compute the target's biggest content-feature gaps vs its named peers and
    return them ranked by expected impact. Output is a punch list: each gap
    has the leader brand, peer-average, research-backed impact coefficient,
    and a one-sentence evidence string.
    """
    peers = [p for p in (req.peers or []) if p and p.lower() != req.target.lower()]

    # Pull features for target + peers. Crawls on-demand when the store is empty.
    ...
    # Mention rates from the feature store (zero if /collect hasn't run).
    ...
    # Compute one GapItem per tracked feature where the user lags
    ranked: list[dict] = []
    for feature in RESEARCH_IMPACT.keys():
        ...
        peer_vals = [v for v in peer_vals if v != 0 or feature in ("readability_grade",)]  # keep 0s only for scale-checked features
        if not peer_vals:
            continue
        gap = _compute_gap(feature, user_val, peer_vals)
        if not gap:
            continue
        # Which peer is the leader for this specific feature?
        ...
        # Per-peer values for mini bar chart
        gap["peer_values"] = [
